...
class Dados_pessoais_opcao_assistencia_controller extends MY_controller {
    ...
    public function form($cpf, $id_edital) {
        ...
        $this->view_data['lista_financiamento'] = $this->tipo_financiamento_dao->getAssistencia();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('dados_pessoais_opcao_assistencia_view', $this->view_data);
            ...
            for($i = 0; $i < 3; $i++) {
                ...
                $univ_post_str = "universidade";
                $univ_post_str .= $i + 1;
                $camp_post_str = "campus";
                $camp_post_str .= $i + 1;
                $curso_post_str = "curso";
                $curso_post_str .= $i + 1;
                $edital_has_curso->setEdital_id_edital($id_edital);
                $edital_has_curso->setUniversidade_id_universidade($this->input->post($univ_post_str));
                $edital_has_curso->setCampus_universidade_id_campus_universidade($this->input->post($camp_post_str));
                $edital_has_curso->setCurso_universidade_id_curso_universidade($this->input->post($curso_post_str));
                $edital_has_curso->setId_edital_has_curso($this->edital_has_curso_dao->getIdEditalHasCurso($edital_has_curso));
                
                $campo_str = "opcao";
                $campo_str .= $i + 1;
                
                $this->view_data[$campo_str] = $edital_has_curso->getId_edital_has_curso();
                
                $desc_opcao[$i] = array(
                    $univ_post_str    =>  $this->universidade_dao->getDescricaoUniversidade($edital_has_curso->getUniversidade_id_universidade()),
                    $camp_post_str    =>  $this->campus_universidade_dao->getDescricaoCampus($edital_has_curso->getCampus_universidade_id_campus_universidade()),
                    $curso_post_str   =>  $this->curso_universidade_dao->getDescricaoCurso($edital_has_curso->getCurso_universidade_id_curso_universidade())
                );
                
            }
            
            $this->view_data['desc_opcao'] = $desc_opcao;
            ...
            $this->load->view('dados_pessoais_opcao_assistencia_confirm_view', $this->view_data);
    ...
    public function registrar() {
        ...
        $ficha_curso_has_edital_curso = new Ficha_curso_has_edital_curso_to();
        
        for($i = 0; $i < 3; $i++) {
            
            $index_str = "id_edital_has_curso";
            $index_str .= $i +1;
            
            $ficha_curso_has_edital_curso->setEdital_has_curso_id_edital_has_curso($this->input->post($index_str));
            $ficha_curso_has_edital_curso->setFicha_curso_id_ficha_curso($ficha_curso->getId_ficha_curso());
            
            $this->ficha_curso_has_edital_curso_dao->insertFichaCursoHasEditalCurso($ficha_curso_has_edital_curso);
    ...
    function seguinte() {
        ...
        redirect("pdf_opcao_assistencia_controller/index/$id");
