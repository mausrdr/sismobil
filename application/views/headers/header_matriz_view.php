    ...
    <head>
        ...
        <script>
            ...
            $menu(document).ready(function(){
                ...
                $menu("ul.subnav").parent().append("<span></span>"); //Somente mostra o trigger do drop down quando o js está habilitado (Adicionando uma tag span vazia depois de ul.subnav*)
                
                $menu("ul.topnav li span").click(function() { //Quando o trigger for clicado...
                    
                    //Os eventos a seguir são aplicados ao prórpio subnav(movendo o subnav para cima e para baixo)
                    $menu(this).parent().find("ul.subnav").slideDown('fast').show(); //Dá um drop down no subnav quando clicado
                    
                    $menu(this).parent().hover(function() {
                    }, function(){	
                        $menu(this).parent().find("ul.subnav").slideUp('slow'); //Quando o ponteiro do mouse sai de cima do subnav, ele se recolhe
                    });
                    
                    //Os eventos a seguir são aplicados ao trigger (Eventos hover pro trigger)
                }).hover(function() { 
                    $menu(this).addClass("subhover"); //Quando o ponteiro do mouse estiver em hover, adiciona a classe "subhover"
                }, function(){
                    $menu(this).removeClass("subhover"); //Quando o ponteiro do mouse não estiver em hover, remove a classe "subhover"
                });
                jQuery.noConflict();
                jQuery(function($){
